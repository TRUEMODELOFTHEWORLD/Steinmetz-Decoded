...
Use the [Hysteresis Loop and Steinmetz Loss tool](/tools/#hysteresis-loop-and-steinmetz-loss) to vary `B`, `f`, `eta`, and `n`. The tool keeps the coefficient in adjustable units until the archive completes source-unit verification.
